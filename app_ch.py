...
class ThicknessApp:
    ...
    def start_measurement(self) -> None:
        ...
        if self.velocity_m_s is None:
            self.status_var.set("请先校准，再开始测量")
            return
    ...
    def save_recent_data(self) -> None:
        if not self.saved_thickness:
            self.status_var.set("没有可保存的厚度数据")
            return
        ...
        self.save_button.config(state=tk.DISABLED)
        self.status_var.set(f"已保存 10 行数据到 {path.name}")
    ...
    def start_board_programming(self) -> None:
        ...
        self.start_button.config(state=tk.DISABLED)
        self.stop_button.config(state=tk.DISABLED)
        self.status_var.set("正在配置 L3R 板...")
    ...
    def board_programming_worker(self) -> None:
        try:
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(f"板卡配置失败：{detail}")
    ...
    def update_calibration_inputs(self) -> bool:
        try:
            ...
            self.status_var.set("校准厚度必须是数字")
            return False

        if thickness_value <= 0:
            self.status_var.set("校准厚度必须大于 0 mm")
            return False

        try:
            velocity_value = float(self.approx_velocity_var.get())
        except ValueError:
            self.status_var.set("参考声速必须是数字")
            return False

        if velocity_value <= 0:
            self.status_var.set("参考声速必须大于 0 m/s")
            return False
        ...
        try:
            while True:
                ...
                if kind == "status":
                    ...
                    self.status_var.set("板卡就绪，请进行校准")
                    ...
                    self.status_var.set(f"校准完成，声速 {self.velocity_m_s:.1f} m/s")
                    ...
                    if self.finish_auto_calibration():
                        ...
                        self.status_var.set(
                            f"校准完成，声速 {self.velocity_m_s:.1f} m/s。按“开始”或点击两个底波进行调整。"
                        )
                    else:
                        self.calibration_selecting = True
                        self.calibration_clicks.clear()
                        self.status_var.set("请点击第一次底波，然后点击第二次底波")
    ...
    def update_plot(self, frame: int, trace: np.ndarray, env: np.ndarray, result, velocity_m_s: float) -> None:
        ...
        if result is None:
            ...
            self.readout.set_text("未检测到底波，请调整门限/阈值")
            return
        ...
        self.readout.set_text(
            f"声速：{velocity_m_s:.1f} m/s\n"
            f"底波间隔：{result.dt_s * 1e6:.3f} us\n"
            f"一次-二次底波：{thickness_mm:.3f} mm\n"
            f"仅一次底波：{first_bw_only_mm:.3f} mm"
        )
        self.status_var.set(f"正在测量，厚度 {thickness_mm:.3f} mm")
    ...
    def update_calibration_plot(
        ...
        self.readout.set_text("校准后请选择第一次和第二次底波")
        self.status_var.set(f"正在校准... {usable_frames}/{total_frames} 帧有效")
    ...
    def on_plot_click(self, event) -> None:
        ...
        if self.calibration_clicks and idx <= self.calibration_clicks[0]:
            self.status_var.set("第二次底波必须在第一次底波之后")
            return
        ...
        if len(self.calibration_clicks) == 1:
            self.status_var.set("已选择第一次底波，请点击第二次底波")
            self.canvas.draw_idle()
            return
    ...
    def finish_manual_calibration(self, manual: bool = True) -> None:
        ...
        self.stop_button.config(state=tk.DISABLED)
        self.readout.set_text(
            f"声速：{self.velocity_m_s:.1f} m/s\n"
            f"底波间隔：{dt_s * 1e6:.3f} us\n"
            f"{'手动' if manual else '自动'}校准"
        )
        self.status_var.set(f"校准完成，声速 {self.velocity_m_s:.1f} m/s")
